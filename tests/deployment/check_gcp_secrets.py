...
def main():
    ...
    try:
        ...
        secret_var = os.getenv("ODOO_PROD_URL")

        if secret_var:
            print(
                "✅ Conexión con GCP Secret Manager exitosa (la variable ODOO_PROD_URL fue cargada)."
            )
            return True
        else:
            raise ValueError(
                "La variable de entorno ODOO_PROD_URL no se encontró. La carga desde GCP Secret Manager pudo haber fallado."
            )
