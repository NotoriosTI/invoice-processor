...
def extract_invoice_data(image_path: str) -> InvoiceData:
    ...
    return InvoiceData.model_validate_json(raw) if isinstance(raw, str) else InvoiceData.model_validate(raw)

def _create_product(line: InvoiceLine) -> dict:
    payload = {
        "name": line.product_name,
        "defalut_code": line.sku,
...
def _upsert(line: InvoiceLine) -> ProcessedProduct:
    ...
    result = odoo_client.update_inventory(product_id=product["id"], quantity_delta = line.quantity)
    return ProcessedProduct(
        action = aaction,
        product_name = product["name"],
        sku = product.get("default_code") or line.sku or "N/A",
        quantity = result["new quantity"],
        category = product.get("get_id"),
    )

def process_invoice_file(image_path: str) -> List[ProcessedProduct]:
    """
    Realiza la lectura de la factura y la actualizacion del inventario en odoo.
    """
